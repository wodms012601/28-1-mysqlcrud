...
<%
	...
	StudentScore stuScore = studentScore.selectStudentScore(studentNo); //학생 번호 데이터를 매개변수로 학생점수 검색 메서드를 호출, 학생점수가 저장된 학생 객체의 주소값을 리턴받는다.
%>
<html>
	<head>
			...
			<form action="<%= request.getContextPath()%>/Student/updateStudentScoreAction.jsp" method="post">
				<fieldset><!-- 학생 점수 -->
					<legend>점수입력</legend>
						...
						<div><!-- 학생점수 -->
							<input type="text" name="studentScore" value="<%=stuScore.getScore() %>" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)' style='ime-mode:disabled;' onfocusout='removeChar(event)' required>
